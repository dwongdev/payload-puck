...
export const FULL_WIDTH_TABLE_STYLE: React.CSSProperties = {
  width: '100%',
...
export function centeredTableStyle(maxWidth: number): React.CSSProperties {
  return {
    width: '100%',
...
export function emailAlign(alignment: EmailAlignment | null | undefined): 'left' | 'center' | 'right' {
  return alignment ?? 'left'
}

/** Maps alignment to margin style for block centering */
export function alignmentToMargin(alignment: EmailAlignment | null | undefined): React.CSSProperties {
  switch (alignment) {
    case 'center':
 ...
 */
export function tiptapHtmlToEmailHtml(
  html: string,
  options?: {
    fontSize?: string
    color?: string
  ...
  const result = cleaned
    // Paragraphs
    .replace(/<p>/g, `<p style="${baseStyle};margin-bottom:1em">`)
    .replace(/<p\s/g, `<p style="${baseStyle};margin-bottom:1em" `)
    // Headings
    .replace(/<h1>/g, `<h1 style="${baseStyle};font-size:32px;font-weight:bold;margin-bottom:0.5em">`)
    .replace(/<h2>/g, `<h2 style="${baseStyle};font-size:24px;font-weight:bold;margin-bottom:0.5em">`)
    .replace(/<h3>/g, `<h3 style="${baseStyle};font-size:20px;font-weight:bold;margin-bottom:0.5em">`)
    // Lists
    .replace(/<ul>/g, `<ul style="margin:0 0 1em 0;padding-left:20px;${baseStyle}">`)
    .replace(/<ol>/g, `<ol style="margin:0 0 1em 0;padding-left:20px;${baseStyle}">`)
    .replace(/<li>/g, '<li style="margin-bottom:0.25em">')
    // Links
    .replace(/<a /g, '<a style="color:#007bff;text-decoration:underline" ')
    // Blockquotes
    .replace(
      /<blockquote>/g,
      `<blockquote style="margin:0 0 1em 0;padding:10px 20px;border-left:4px solid #dddddd;${baseStyle}">`
    )
...
export function vmlBackgroundClose(): string {
  return `<!--[if gte mso 9]>
</v:textbox>
</v:rect>
<![endif]-->`
